...
export function ValidationPanel() {
  ...
  const runChecks = useCallback(async () => {
    ...
    const DEFAULT_TEMPLATE_API_KEY = "27b4f19d-bc6b-4bcf-8d54-1af1107b653a";
    ...
    try {
      ...
      const key = (brazeMod.BRAZE_API_KEY ?? "").trim();
      const url = (brazeMod.BRAZE_BASE_URL ?? "").trim();
      const keyMissing =
        !key ||
        key === "YOUR_BRAZE_API_KEY" ||
        key === DEFAULT_TEMPLATE_API_KEY;
      const urlMissing = !url || url === "YOUR_SDK_ENDPOINT";
      brazePlaceholder = keyMissing || urlMissing;
      if (brazePlaceholder) {
        const missing = [
          keyMissing ? "API key (missing or default template value)" : "",
        ...
        checks.push({
          ...
          status: "fail",
          detail: `Missing Braze ${missing}. Open lib/braze.ts and replace placeholder/default template values. See FIXES.md #0.`,
        });
      } else {
        checks.push({
          id: "braze-config",
    ...
    try {
      console.log("[v0] Fetching /api/scan-source...");
      const res = await fetch("/api/scan-source");
      console.log("[v0] scan-source response status:", res.status);
      scanData = await res.json();
      console.log("[v0] scan-source data received:", scanData ? "success" : "empty");
    } catch (err) {
      console.error("[v0] scan-source fetch error:", err);
      scanError = true;
    }
    ...
    // ---------------------------------------------------------------
    {
      ...
      // Check intended CSP
      try {
        console.log("[v0] Fetching /api/check-csp...");
        const res = await fetch("/api/check-csp");
        console.log("[v0] check-csp response status:", res.status);
        const body = await res.json();
        const csp: string = body.intendedCsp ?? "";
        intendedOk = csp.includes("frame-ancestors") && csp.includes(REQUIRED_ORIGIN);
      } catch (err) {
        console.error("[v0] check-csp fetch error:", err);
        intendedError = true;
      }

      // Check observed headers
      try {
        console.log("[v0] Fetching /api/check-headers...");
        const res = await fetch("/api/check-headers");
        console.log("[v0] check-headers response status:", res.status);
        const body = await res.json();
        if (!body.error) {
