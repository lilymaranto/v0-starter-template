...
// Callers import { startWebSession, setUser, listenForNative, webReady } from this file.
...
declare global {
  interface Window {
    DemoBridge?: {
      ...
      /** Injected by Doppel `demo_bridge_utility.js`; posts `webReady` to native for handshake. */
...
export function startWebSession(opts: {
  userId: string;
  configId?: string | null;
}) {
  const { userId, configId } = opts;
...
/**
 * Call after the Braze Web SDK is ready so the Doppel WKWebView can run the
 * deferred handshake (`webReady` message → native `startSession` / `syncUser`).
 * Wraps `DemoBridge.notifyWebReady`; no-op without the injected bridge.
 */
export function webReady(detail?: {
  configId?: string | null;
  userId?: string | null;
}): void {
  if (!hasBridge()) return;
  const notify = window.DemoBridge?.notifyWebReady;
  if (typeof notify !== "function") return;
  try {
    notify(detail ?? {});
  } catch (error) {
    console.warn("[Bridge] notifyWebReady failed:", error);
  }
}
